"""Reference (unoptimized) Cascade implementation — frozen for verification.

This file is the un-optimized Cascade kept verbatim alongside the optimized
`cascade.py` so that performance optimizations (B1, B2, B5, B6) can be
validated to produce bit-identical FrameResults. See
`docs/optimization-brief.md` and `scripts/check_cascade_optimization.py`.

Do not modify this file when applying optimizations to `cascade.py`. It is
